// adapt from https://github.com/wenyan-lang/wenyan/blob/master/src/render.ts
    ...
    if (txt[i] == "\n") {
      ...
      if (!iden && txt[i] == "批" && txt[i + 1] == "曰") {
            ...
            for (var k = 0; k < cmt.length; k++) {
              if (cmt[k] == "。" || cmt[k] == "、") {
                ...
                svg += `<text x="${kx}" y="${ky}" font-size="${CS /
                  2.2}" font-family="${FONT}" fill="${BOOK_COLORS.comment}">${cmt[k]
                  }</text>`;
                ky += CH / 1.5;
                if (ky > Y0 - PCB) {
                  ky = PCT + CH / 2;
        ...
        svg += `<circle cx="${x + CW - 3}" cy="${y - CH + PSH}" r="${CH /
          7}" stroke="${RED}" fill="none" stroke-width="1" wy-data="${txt[i]
          }"></circle>`;
      } else if (txt[i] == "、" && !iden) {
        ...
        svg += `<path d="M${x} ${y - CH / 2 + h / 2 + PSH} L${x + CW / 5} ${y -
          ...
          CH / 2 +
          h / 2 +
          PSH}" stroke="${BLACK}" fill="none" stroke-width="0.5" wy-data="${txt[i]
          }"></path>`;
        ...
        svg += `<text x="${x +
          CO}" y="${y}" font-size="${CS}" font-family="${FONT}" fill="${BOOK_COLORS[sm[i]]
