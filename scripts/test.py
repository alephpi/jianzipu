...
def test_abbr_simple_form_parse_and_str():
  ...
  for i in l:
    assert parse(i, form='abbr').to_string('abbr') == i
...
def test_transcribe():
  ...
  for i, j in zip(ortho, abbr):
    assert parse(i, form='ortho').to_string('abbr') == j

  for i, j in zip(abbr, ortho):
    assert parse(i, form='abbr').to_string('ortho') == j
