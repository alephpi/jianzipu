...
XIAN_FINGER = sorted(GLYPHS['弦序指法'].keys(), key=len, reverse=True)
MOVE_FINGER = sorted(GLYPHS['走位指法'].keys(), key=len, reverse=True)
SPECIAL_FINGER = sorted(GLYPHS['特殊指法'].keys(), key=len, reverse=True)
MODIFIER = sorted(GLYPHS['修饰'].keys(), key=len, reverse=True)
JOINT_FINGER = sorted(GLYPHS['联袂指法'].keys(), key=len, reverse=True)
COMPLEX_FINGER = sorted(GLYPHS['复式指法'].keys(), key=len, reverse=True)
MARKER = sorted(GLYPHS['记号'].keys(), key=len, reverse=True)

NUMBER = ['十一','十二','十三','一','二','三','四','五','六','七','八','九','十','外','半']
ALL = HUI_FINGER + XIAN_FINGER + MOVE_FINGER + SPECIAL_FINGER + JOINT_FINGER + COMPLEX_FINGER + MODIFIER + MARKER + NUMBER
